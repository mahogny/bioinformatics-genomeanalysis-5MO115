...
- **QE1.2** A read of length L is broken into k-mers. Show that there are L − k + 1 k-mers per read. Derive the relationship `k-mer coverage = read coverage × (L − k + 1) / L`, and explain why a sequencer that gives you 100× *read* coverage may give you only 80× k-mer coverage at k = 31.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
A k-mer is a window of size k sliding along the read. How many distinct positions can the window start at, given the window must fit inside the read?
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
A k-mer can start at positions 1, 2, ..., L − k + 1 (any later start runs off the read end). So there are exactly L − k + 1 k-mers per read.

Total k-mers = (number of reads) × (L − k + 1). Genome size G is fixed. Read coverage c_read = (reads × L) / G. K-mer coverage c_kmer = (reads × (L − k + 1)) / G = c_read × (L − k + 1) / L.

For L = 150 and k = 31: factor = 120 / 150 = 0.8, so 100× read coverage → 80× k-mer coverage. Assemblers care about *k-mer* coverage, not read coverage, so trimming reads or picking a large k can quietly starve the assembler of evidence.
:::
...
- **QE1.4** Construct the (k = 3) de Bruijn graph for `ATGGAAGTC`. List the (k−1)-mer nodes and the directed edges (each edge is a k-mer). Walk it and confirm you recover the original sequence.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
For k = 3, each node is a 2-mer (prefix or suffix of a 3-mer). Each 3-mer becomes an edge from its first 2 letters to its last 2 letters.
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
3-mers in `ATGGAAGTC` (7 of them): `ATG, TGG, GGA, GAA, AAG, AGT, GTC`.

Nodes (2-mers visited): `AT, TG, GG, GA, AA, AG, GT, TC`.
...
GT -> TC    (from GTC)
```
Walking from `AT` along all 7 edges spells out `AT → TG → GG → GA → AA → AG → GT → TC`. Concatenating the first letter of each node plus the suffix of the last node gives `A T G G A A G T C` — the original sequence. ✓
:::
...
- **QE1.8** Using the Lander–Waterman result, P(base uncovered) = e^−c for coverage c. What fraction of the genome is uncovered at c = 5×? At c = 30×? At c = 200×? Why does extra coverage past ~50× give diminishing returns for assembly?

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
e^−5, e^−30, e^−200 — three very different orders of magnitude.
:::
...
c <- c(5, 30, 200)
p_uncov <- exp(-c)
data.frame(coverage = c, fraction_uncovered = p_uncov,
           bases_missing_in_5Mb = p_uncov * 5e6)
```

- 5× → ~0.67 % uncovered (~33 kb of a 5 Mb genome).
- 30× → ~9 × 10⁻¹⁴ — effectively zero bases uncovered.
- 200× → ~10⁻⁸⁷ — astronomically zero.

Past ~30–50× the random-uncovered-base concern is gone; remaining assembly problems come from **non-random** issues that more coverage doesn't fix: high-GC dropout, secondary structure, low-complexity regions, repeats longer than the read. Extra coverage past this point doesn't extend contigs — it just makes assembly slower and pushes the assembler toward over-merging error variants. Many bacterial assemblers (including SKESA) actually *downsample* high-coverage data.
:::

- **QE1.9** Single reads can't span a repeat longer than the read length. How does paired-end information help bridge such a repeat in the de Bruijn graph? What insert size would you choose for a genome whose longest repeat is ~700 bp?
...
(For repeats > 1 kb, the standard answer is to add a long-read library — see QE1.10.)
:::

- **QE1.10** Short Illumina reads have high per-base accuracy (Q30+) but limited span; Nanopore/PacBio reads have lower per-base accuracy (Q10–20 raw) but span tens of kb. Sketch a workflow that uses **both** and explain what each contributes.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
"Hybrid assembly" — use long reads to lay down a scaffold and short reads to polish each base.
:::
