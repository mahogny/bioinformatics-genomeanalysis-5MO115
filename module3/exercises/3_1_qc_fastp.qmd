...
- **QE1.6** FastQC reports a sharp secondary peak at 65 % GC for what should be an *E. coli* sample (expected ~50 % GC, single Gaussian). Give two plausible hypotheses — one biological, one technical — and a quick test for each.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
"Sharp" and "secondary" are the key words: the contaminant is a single, defined species — not a smear.
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
- **Biological — contamination by a higher-GC organism.** Mycobacteria (~65 % GC), some Actinobacteria, or human DNA (~41 % isn't a match, but human reads can still cluster oddly). Test: run a few thousand reads through Kraken2 / mash screen against a contaminants database; or BLAST a handful of unmapped reads.
- **Technical — over-amplified high-GC fragments or barcode/adapter read-through.** A PCR-biased amplicon, a UMI/index read leaking into Read 1, or unremoved adapter dimers can all give a tight non-50% peak. Test: rerun FastQC *after* fastp adapter/quality trimming; check "Overrepresented sequences" for known adapter or contaminant motifs; inspect duplication levels.

Either way, **don't** just trim the peak away — figure out where it came from before downstream analysis.
:::
...
- **QE1.9** A library was sequenced 2×150 bp with a mean insert size of 250 bp. What fraction of read pairs do you expect to overlap, and what should you do about overlaps before downstream analysis?

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
A pair overlaps when (read length × 2) > insert size, i.e. when insert < 300 bp.
:::

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
Insert sizes are typically log-normal or roughly Gaussian; if the mean is 250 bp with SD ~50 bp, what fraction lies below 300 bp?
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
Pairs overlap whenever the insert is shorter than 2 × read length = 300 bp. For a Gaussian with mean 250 and SD 50 bp:
```{r, eval=TRUE}
pnorm(300, mean = 250, sd = 50)
```
~84 % of pairs overlap. Options for handling them:

- **Merge** with `fastp --merge` (or `bbmerge`, `PEAR`, `FLASH`): produces a single longer "merged" read with averaged quality at overlapping positions — useful for amplicon work where the overlap *is* the signal.
- **Leave un-merged and let the aligner handle it.** Mappers (BWA-MEM, bowtie2) take read pairs as input and reconcile overlaps when computing alignment scores; downstream variant callers will see the same base twice and weight it accordingly.
- **Soft-clip with `fastp --overlap_diff_limit`** if you only want to use the overlap to *correct* low-quality 3′ bases.

For variant calling on small inserts, merging is usually the right call.
:::
