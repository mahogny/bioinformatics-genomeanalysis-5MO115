...
Theoretical questions on read mapping, SAM/BAM, VCF, and variant filtering. Pen-and-paper unless a code chunk is shown.

- **QE1.1** A VCF record has `QUAL=30`. Using the Phred definition, what is the probability the variant call is wrong? What about `QUAL=200`? Why do callers like bcftools produce QUAL values much greater than 60 even though Phred values from a single read top out around Q40?

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
QUAL is on the same Phred scale: $P(\text{wrong}) = 10^{-Q/10}$. Multiple independent reads supporting the same call combine multiplicatively.
:::
...
10^(-30/10); 10^(-200/10)
```
QUAL=30 → 0.1 % chance the call is wrong; QUAL=200 → $10^{-20}$. QUAL combines evidence from many reads — 20 independent reads each giving Q20 evidence gives an aggregated QUAL ≈ 400 in the absence of priors. Variants supported by 30× clean coverage routinely score QUAL > 200 even though no single base call is that confident.
:::

- **QE1.2** Explain the difference between **MAPQ** (column 5 of a SAM record) and the **per-base Phred quality** (in FASTQ / SAM column 11). Why can a read have all base qualities ≥ Q30 yet MAPQ = 0?

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
Base Q answers "did the sequencer read this base right?". MAPQ answers a different question entirely.
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
- **Base Q** is the sequencer's confidence in each individual base call: pre-alignment, position-local.
- **MAPQ** is the *aligner's* confidence that this read is mapped to the **correct location** in the reference. MAPQ = −10 log₁₀ P(the read actually came from somewhere else).

A read with perfect base quality but mapping to a repeat (e.g. an rRNA operon, or a multicopy IS element) has many equally-good alignment targets — so MAPQ collapses to 0 because the aligner can't tell which copy the read truly came from. Variant callers usually require MAPQ ≥ 20 or MAPQ ≥ 30 by default, which silently filters out repeat-region variants.
:::

- **QE1.3** What do these CIGAR strings tell you about the alignment? `100M`, `98M2S`, `50M3D47M`, `75M5I20M`, `25S75M`.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
M = aligned (match or mismatch), S = soft-clipped, I = insertion in read, D = deletion from read. Numbers are lengths.
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
- `100M` — full-length 100-bp read aligned end-to-end (matches *or* mismatches, CIGAR doesn't distinguish).
- `98M2S` — 98 bp aligned, the last 2 bp soft-clipped (not aligned, but kept in the read sequence). Usually means the read ran into adapter or low-quality bases at the 3′ end.
- `50M3D47M` — 50 bp aligned, then a 3-bp **deletion** in the read relative to the reference, then 47 bp aligned. Total reference span = 100 bp, total read length = 97 bp.
- `75M5I20M` — 75 bp aligned, then 5 bp **inserted** in the read (not in the reference), then 20 bp aligned. Total read length = 100 bp, reference span = 95 bp.
- `25S75M` — first 25 bp soft-clipped, then 75 bp aligned. Often indicates a chimeric / split read.
:::

- **QE1.4** A VCF FORMAT field reports `DP=42, AD=2,40, GQ=99`. What is the variant allele fraction? Is this a confident **homozygous variant** call? What changes if `AD=20,22`?

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
AD = allelic depths (ref, alt). DP = total reads at site. GQ = genotype-call Phred quality.
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
- `AD=2,40`: 2 ref reads, 40 alt reads → **VAF = 40/42 ≈ 95 %**. Combined with GQ=99, this is a confident homozygous-alt call (1/1). The 2 ref reads are most likely sequencing errors.
- `AD=20,22`: VAF = 22/42 ≈ 52 %. In a *haploid* bacterium this is suspicious — you'd expect 0 % or 100 %. Possible causes: within-host heterogeneity (a mixed population), contamination, paralog / repeat mis-mapping, or copy-number variation. In a *diploid* eukaryote this would just be a clean heterozygous (0/1) call.
:::

- **QE1.5** Why does reference-based SNP calling miss variants in regions that are highly divergent from the reference? Give **one** strategy to mitigate this.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
What does BWA do with a read that has 15 mismatches in a 100 bp window?
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
BWA-MEM (and similar) require some minimum identity (typically ~70 %) for a read to be aligned at all. In a region that has diverged a lot from the reference, reads fail to align and the variant caller sees zero coverage — it can't call what it can't see. The same happens for accessory-genome content that simply isn't in the reference.

Mitigation strategies (any one of):

- **Pick a closer reference.** Use a strain-specific or pangenome reference rather than a distant type strain.
- **Reference-free / k-mer SNP calling.** Tools like kSNP or Snippy operate on assemblies; whole-genome aligners (e.g. Parsnp) avoid the per-read-must-map requirement.
- **Iterative consensus refinement.** Build a consensus from initial mapping, re-map against it, repeat. Pulls in variants the first round missed.
- **De novo assemble + map contigs back** with a more permissive aligner like nucmer.
:::

- **QE1.6** bcftools `mpileup | call` combines evidence across reads at each position to call genotypes. Briefly explain the underlying probabilistic model: what does the *likelihood* combine, and what is the *prior*?

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
Bayes: posterior ∝ likelihood × prior. The likelihood is over the read data, the prior over the genotype.
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
For each candidate genotype G ∈ {0/0, 0/1, 1/1} (or just {ref, alt} for haploids), bcftools computes:

- **Genotype likelihood** P(reads | G) — multiplies, across all reads covering the site, the probability of seeing the observed base given that the true genotype is G and given that base's Phred quality. A high-Q read agreeing with G contributes a large factor; a high-Q read disagreeing pushes the likelihood down.
- **Prior** P(G) — a population-level prior, by default ~10⁻³ for variant genotypes. Configurable via `-P` (mutation rate). For haploid mode (`--ploidy 1`), the prior is over {ref, alt}.

The posterior is then the **PL** field in the VCF (Phred-scaled likelihoods relative to the best genotype), and QUAL is the Phred-scaled probability that the site is *not* monomorphic.
:::

- **QE1.7** The same indel can appear at different positions in different VCF files (e.g. `AAA → AA` reported at position 100 in one VCF and position 102 in another). Why does this happen, and what does `bcftools norm -f ref.fa` do about it?

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
Where does a deletion of one `A` from `AAAA` "really" occur — at position 1, 2, 3, or 4?
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
In a homopolymer (or any low-complexity) tract, the deletion is **ambiguous** — multiple equivalent representations give the same final sequence. Different callers anchor the deletion at different positions, so naive comparison of two VCFs misses that they describe the same event.

`bcftools norm -f ref.fa` performs **left-normalization**: shifts each indel as far left as possible (to the smallest position) and trims unnecessary common bases, producing a canonical representation. After `norm`, two VCFs describing the same event have identical CHROM/POS/REF/ALT and can be compared directly. Always normalize before benchmarking, merging, or annotating VCFs.
:::

- **QE1.8** You filter a VCF with `vcftools --minQ 20 --max-missing 0.9 --min-meanDP 10`. What does each flag defend against, and what cost does each impose?

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
For each flag: what would happen if you didn't apply it?
:::
...
|------|----------------|------|
| `--minQ 20` | False-positive variant calls (QUAL = Phred of "this is a real variant"); without it, you keep many noise calls. | Loses real but weakly supported low-frequency variants, especially at sites with low local coverage. |
| `--max-missing 0.9` | Sites called in too few samples — these add noise and are uninformative for downstream phylogeny / GWAS. | In studies with samples of very different coverage, this can systematically drop the regions hardest to sequence (high-GC, repeat-near). |
| `--min-meanDP 10` | Sites where average depth is too low across samples (likely mapping-artefact sites or accessory regions partially missing in many samples). | Filters out true variants in low-coverage genomic regions; can mask copy-number-variable accessory content. |
:::

- **QE1.9** Why is it important to mark / remove PCR duplicates **before** variant calling? Sketch what goes wrong if you don't.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
A PCR duplicate is *not* independent evidence — but the variant caller doesn't know that.
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
The probabilistic model assumes each read is an independent draw from the population of fragments at that locus. PCR duplicates violate that assumption: they are clones of a single original fragment. If that single fragment happened to carry a sequencing error, *every* duplicate of it will carry the same error — the caller sees, say, 12 reads supporting an "alt" allele and concludes it's real, when in reality it's a single error replicated 12 times.

Practical consequences:

- **False-positive variants** at PCR-duplicate-heavy sites.
- **Inflated coverage** numbers (and therefore inflated QUAL scores) that don't reflect true sampling depth.
- **Skewed VAF** estimates if duplicates carry one allele preferentially.

`samtools markdup` or `picard MarkDuplicates` flags them (SAM flag 0x400); callers respect the flag and downweight or skip them. For PCR-free libraries this step is less critical but still recommended.
:::

- **QE1.10** Tools like kSNP and Snippy can call SNPs **without** mapping reads to a reference — directly from k-mers (kSNP) or from assemblies + reference (Snippy). What's the trade-off vs traditional read-mapping (BWA + bcftools)?

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
What kinds of variants does read-mapping miss? What kinds does k-mer SNP calling miss?
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
**K-mer / reference-free SNP calling gains:**

- No reference-bias toward a specific strain; finds variants in accessory genes that aren't in any single reference.
- No alignment step → very fast for large cohorts; scales linearly.
- Robust to highly divergent strains where reads fail to map.

**It loses:**

- **Resolution** — a k-mer SNP is a SNP-with-context; tools struggle with multiple SNPs within the same k bp window (which collapse into a single k-mer mismatch).
- **Indels** — k-mer SNP callers typically only report point mutations; structural variants are invisible or have to be inferred indirectly.
- **Per-site quality / depth statistics** — there's nothing equivalent to MAPQ, AD, or GQ. Filtering is coarser.
- **Phasing** — read-mapping preserves which alleles co-occur on the same read; k-mer methods don't.

**Rule of thumb:** for diverse cohorts of bacterial isolates where the goal is "make a SNP-distance tree", k-mer methods (mashtree, kSNP) are an excellent fast choice. For careful, single-strain variant calling — especially anything involving indels, low-frequency variants, or per-site QC — stick with BWA + bcftools.
:::
