...
If you are using RStudio on your own, you can download the file via this link: [clinvar_chr_info_meta.vcf.gz](http://beagle.henlab.org/public/course_introtobioinfo/clinvar_chr_info_meta.vcf.gz)
...
Note that you will need to modify this command a bit depending on where you stored the file, and whether you uncompressed it or not (.gz means that it is gzip-compressed. read.table can handle .gz).
...
- **QE1.1** You have a tab-separated file called `counts.tsv` that has a header on the first line. Write the `read.table()` call that imports it into a data frame called `counts`.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
The default separator of `read.table()` is whitespace. To force tab as the separator, use `sep="\t"`. The header argument controls whether the first row is treated as column names.
:::
...
- **QE1.2** You receive a CSV file `samples.csv` where the first 5 lines are comments produced by the sequencing facility, and missing values are encoded as `"NA"` or `"-"`. Read it into R so that both encodings are treated as missing values, and skip the comment lines.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
Look at the arguments `skip`, `na.strings` and `header` in `?read.table` (or `?read.csv`).
:::

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
`na.strings` accepts a vector of strings, e.g. `na.strings = c("NA", "-")`.
:::
...
- **QE1.3** You have a file `expression.txt` without a header line, where columns are separated by semicolons. Read it in and then set sensible column names afterwards (`gene`, `sample1`, `sample2`, `sample3`).

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
Use `header = FALSE` so R does not interpret the first row as column names. You can then assign names using `colnames()` after the import.
:::

::: {.callout-note collapse="true" appearance="minimal"}
## Solution
```{r, eval=FALSE}
expr <- read.table("expression.txt", header = FALSE, sep = ";")
colnames(expr) <- c("gene", "sample1", "sample2", "sample3")
head(expr)
```
:::
...
- **QE1.4** Load the built-in `mtcars` dataset and inspect it: print the first rows, find out the number of rows and columns, look at the structure of each column, and print a summary of the numerical values.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
Useful functions: `head()`, `nrow()`, `ncol()`, `dim()`, `str()`, `summary()`. `mtcars` is built into R so you do not need to read anything from disk.
:::
...
- **QE1.5** Using the built-in `mtcars` dataset, make a scatter plot of car weight (`wt`) on the x-axis and fuel consumption (`mpg`) on the y-axis using base R. Add meaningful axis labels and a title.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
Use `plot(x, y, xlab=..., ylab=..., main=...)`. You can access columns of `mtcars` with `mtcars$wt` and `mtcars$mpg`.
:::
...
plot(mtcars$wt, mtcars$mpg,
     ...
     main = "Fuel consumption vs. car weight")
```
:::

- **QE1.6** Still using `mtcars`, draw a histogram of `mpg` and a boxplot of `mpg` split by the number of cylinders (`cyl`).

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
For a histogram, use `hist()`. For a boxplot split by a grouping variable, use the formula interface: `boxplot(mpg ~ cyl, data = mtcars)`.
:::
...
- **QE1.7** Using the built-in `iris` dataset, make a ggplot scatter plot of `Sepal.Length` (x) vs. `Petal.Length` (y), and colour the points by `Species`.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
Map the colour aesthetic inside `aes()`: `aes(x = Sepal.Length, y = Petal.Length, colour = Species)`. Then add `geom_point()`.
:::
...
::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
Use `facet_wrap(~ Species)` to create one panel per level of `Species`.
:::
...
- **QE1.9** Imagine you have a data frame `gene_expr` with a column `counts` containing raw gene expression counts for thousands of genes. Most genes have low counts and a few have very high counts. Plot a histogram of the counts on a log10 x-axis so that you can see the distribution clearly.

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
A common approach is to add a small pseudo-count (e.g. `+1`) before taking the log so genes with `counts == 0` do not disappear. In ggplot you can use `scale_x_log10()` directly.
:::

::: {.callout-tip collapse="true" appearance="minimal"}
## Hint
With base R: `hist(log10(gene_expr$counts + 1))`. With ggplot: `geom_histogram()` together with `scale_x_log10()`.
:::
...
```{r, eval=FALSE}
# Base R version
hist(log10(gene_expr$counts + 1),
     xlab = "log10(counts + 1)",
     main = "Distribution of gene expression counts")

# ggplot2 version
library(ggplot2)
ggplot(gene_expr, aes(x = counts + 1)) +
       geom_histogram(bins = 50) +
       scale_x_log10() +
       labs(x = "counts (log10 scale)",
