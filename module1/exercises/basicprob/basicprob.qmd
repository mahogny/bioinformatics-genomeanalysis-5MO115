...
::: {.callout-note title="Biological application: Mendelian inheritance"}
Two heterozygous parents ($Aa \times Aa$) each pass one of their two alleles to the offspring with equal probability. The four equally likely allele combinations are $AA, Aa, aA, aa$, so:
$$P(\text{homozygous recessive } aa) = \tfrac{1}{2} \cdot \tfrac{1}{2} = \tfrac{1}{4}$$
$$P(\text{at least one dominant } A) = 1 - P(aa) = \tfrac{3}{4}$$
(the complement trick from Q1.5, applied to genetics).

For two unlinked genes inherited independently ($AaBb \times AaBb$):
$$P(\text{double recessive } aabb) = \tfrac{1}{4} \cdot \tfrac{1}{4} = \tfrac{1}{16}$$
This is the source of the classic 9:3:3:1 ratio in dihybrid crosses.
:::

::: {.callout-note title="Biological application: mutations along a stretch of DNA"}
Each base in a genome mutates per generation with some small probability $\mu$ (for humans, $\mu \approx 10^{-8}$). Treating bases as independent:
$$P(\text{no mutation in } n \text{ bases}) = (1-\mu)^n$$
$$P(\text{at least one mutation in } n \text{ bases}) = 1 - (1-\mu)^n$$

For $\mu = 10^{-8}$ and $n = 3 \times 10^9$ (a human genome): $P(\geq 1) \approx 1 - e^{-30} \approx 1$ — every offspring carries new mutations. This is the same complement-of-"at least one" calculation as Q1.5, just at genome scale.
:::

::: {.callout-note title="Biological application: matching a random k-mer"}
Pick a random DNA sequence of length $k$ (a "k-mer") and ask whether it matches a fixed target sequence. Assuming each base is equally likely (A/C/G/T) and bases are independent:
$$P(\text{match}) = \left(\tfrac{1}{4}\right)^k$$

For $k = 4$ this is $1/256$; for $k = 20$ it is $\approx 10^{-12}$. This is why PCR primers (typically 18–25 bp) are usually unique in a genome — by random chance, a 20-mer would be expected to occur fewer than once in a $3 \times 10^9$-base human genome. Same independence-multiplication as Q1.3 (four heads in a row), just over four outcomes instead of two.
:::
...
::: {.callout-note title="Biological application: expected mutations per generation"}
Each base mutates with small probability $\mu$ per generation. The expected number of new mutations across the whole genome of length $L$ is
$$E[\#\text{mutations}] = \mu \cdot L$$
For humans ($\mu \approx 10^{-8}$, $L \approx 3 \times 10^9$ bp), this gives $E \approx 30$ new mutations per offspring per generation — and indeed, family trio sequencing studies estimate $\sim 60$ *de novo* mutations per child (counting both parental contributions). Linearity of expectation makes this a one-line calculation: each base is a tiny "lottery ticket" with payoff 1 if it mutates, 0 otherwise, and you just sum the expected payoffs.
:::

::: {.callout-note title="Biological application: expected sequencing coverage"}
A short-read sequencer produces $N$ reads of length $\ell$ each, scattered roughly uniformly over a genome of size $L$. The expected number of reads covering any given base — the "expected coverage" — is
$$E[\text{coverage}] = \frac{N \cdot \ell}{L}$$
For example, $N = 3 \times 10^8$ reads of $\ell = 100$ bp on a human genome ($L = 3 \times 10^9$ bp) gives $E[\text{coverage}] = 10\times$. You will use this exact formula in module 3 when planning sequencing depth.
:::
